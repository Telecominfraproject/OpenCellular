...
namespace cnip.pages
{
    public partial class webui : Page
    {
        ...
        [WebMethod]
        public static string GetRadioPlanResult()
        {
            string msg = "";
            string puid = (string)HttpContext.Current.Session["uid"];
            DataTable dt = GetDataTableFromQuery("SELECT resultstring FROM pusersresults WHERE " +
                "uid='" + puid + "' AND resultid='0' AND resulttype='radioplan';");
            if (dt.Rows.Count > 0) { msg = dt.Rows[0][0].ToString(); }
            return msg;
        }
        /***************************************************************/
        ...
        public static void GetLinksFromDB()
        {
            ...
            var options = new[] {
                    "-f", "GEOJSON",
                    "-sql", "SELECT " +
                    "linkid, linkname, linktype, siteida, locheighta, heighta, bearinga, " +
        ...
        {
            string puid = (string)HttpContext.Current.Session["uid"];
            DataTable ni = GetDataTableFromQuery("SELECT MAX(CAST(siteid AS INTEGER))+1 FROM " +
                "pusersnetwork WHERE uid='" + puid + "' GROUP BY uid");
            int newid = 1; if (ni.Rows.Count > 0) { newid = ni.Rows[0][0].ToString().ToInt(); }
            return newid.ToString();
        }
        [WebMethod]
        public static void UploadUserNetworkToDB(string sitename)
        {
            if (_IsPukey())
            {
                string puid = (string)HttpContext.Current.Session["uid"];
                var options = new[] {
                    "-overwrite",
                    "-f", "PostgreSQL",
                    "-nln", "myusernetwork_" + puid,
                    "-nlt","POINT"};
                Json2Pgsql(GetVectorLayersPath() + sitename,
                    "PG:dbname=cnip host=localhost user=postgres password=root port=5432", options);
                if (TableExists("myusernetwork_" + puid))
                {
                    if (TableHasData("myusernetwork_" + puid))
                    {
                        string siteid = GetNewSiteId();
                        try
                        {
                            ExecuteNonQuery("INSERT INTO pusersnetwork " +
                                "(uid, siteid, sitename, longitude, latitude, height, region, " +
                                ...
                                "vbeamwidth, hbeamwidth, downtilt, antennagain, feederloss, wkb_geometry) " +
                                "SELECT '" + puid + "', " +
                                "CAST(" + siteid + " + row_number() over () AS character varying), " +
                                "sitename, longitude, latitude, height, region, " +
                                "country, city, district, province, address, comments, technology, " +
                                "band, bandwidth, cellid, lac, rfcn, rfid, dlfrequency, ulfrequency, " +
                                "rfpower, hba, azimuth, antennamodel, antennatype, polarization, " +
                                "vbeamwidth, hbeamwidth, downtilt, antennagain, feederloss, wkb_geometry " +
                                "FROM myusernetwork_" + puid + ";");
                        }
                        catch (Exception) { }
                    }
                    DropTable("myusernetwork_" + puid);
                    File.Delete(GetVectorLayersPath() + sitename);
                }
            }
        }
                ...
                if (TableExists("mylinks_" + puid))
                {
                    ...
                    if (TableHasData("mylinks_" + puid))
                    {
                        ExecuteNonQuery("INSERT INTO puserslinks " +
                            "(uid, linkid, linkname, linktype, siteida, locheighta, heighta, bearinga, " +
                            "channelwidtha, frequencya, outputpowera, antennagaina, lossesa, " +
                            "siteidb, locheightb, heightb, bearingb, channelwidthb, frequencyb, " +
                            "outputpowerb, antennagainb, lossesb, distance, elevstr, name, email, wkb_geometry) " +
                            "SELECT '" + puid + "', " +
        ...
        [WebMethod]
        public static void RadioPlan(string resultid, string sites)
        {
            if (_IsPukey())
            {
                string args = "";
        ...
        [WebMethod]
        public static void Links(string resultid, string sites)
        {
            if (_IsPukey())
            {
                string args = "";
