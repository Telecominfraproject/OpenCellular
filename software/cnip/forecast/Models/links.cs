...
namespace forecast.Models
{
    ...
    {
        public static void Forecast(string puid, string tempfol, string sites, bool analysis = false)
        {
            try
            {
                // delete any associated internal links to selected sites
                ...
                for (int i = 0; i < edges.Length; i++)
                {
                    links += CreateLink(new CreateLinkOptions(
                        ...
                        puid, sitesList[edges[i].dest], "internal"));
                }

                // create internal and public link within 50Km of selected sites 
                // to nearest site from selected sites 
                Site internalSite = GetClosestSite(new GetClosestSiteOptions(
                    puidWhere, NotSitesWhere, center, within50KmsOfSites));
                if (internalSite.Siteid.Length > 0)
                {
                    // create link, find nearest selected site to 
                    //(find nearest site from internal network within 50Kms of selected sites)
                    Site selectedSite = GetClosestSite(new GetClosestSiteOptions(
                        puidWhere, sitesWhere, internalSite.Location, within50KmsOfSites));
                    links += CreateLink(new CreateLinkOptions(puid, selectedSite.Siteid,
                        puid, internalSite.Siteid, "internal"));
                }
                ...
                string resultString = AdjustHeightsToMeetLinks(puid, links);
        ...
        private static string AdjustHeightsToMeetLinks(string puid, string links)
        {
            ...
            bool linksmeet = false;
            List<string> internalSitesModified = new List<string>();
            ...
            while (!linksmeet)
            {
                ...
                for (int i = 0; i < links.Split(',').Length; i++)
                {
                    ...
                    "linkid ='" + links.Split(',')[i] + "';").Rows)
                    {
                        ...
                        locheightBm = row[4].ToString().ToDouble();
                        xDistanceKm = row[5].ToString().ToDouble();
                        xFrequencyGHz = row[6].ToString().ToDouble();
                        xElevations = row[7].ToString().Split('@');
                    ...
                    deviceheightAm = siteheightAm + locheightAm;
                    deviceheightBm = siteheightBm + locheightBm;
                    ...
                    if (xObstm > 0)
                    {
                        if (linktype == "public")
                        {
                            double xObsFactor = ((xObstm * 2)) - deviceheightAm - deviceheightBm;
                            if (Math.Round(xObsFactor) > 0)
                            {
                                SetSiteHeight(puid, siteida, siteheightAm + xObsFactor);
                                linksmeet = false;
                                publicSitesModified.Add(siteida);
                            }
                            else
                            {
                                SetSiteHeight(puid, siteida, siteheightAm + 0.5);
                                linksmeet = false;
                                publicSitesModified.Add(siteida);
                            }
                        }
                        ...
                        {
                            if (Math.Round(deviceheightAm) < Math.Round(xObstm))
                            {
                                SetSiteHeight(puid, siteida, xObstm - locheightAm);
                                linksmeet = false;
                                internalSitesModified.Add(siteida);
                            }
                            else
                            {
                                SetSiteHeight(puid, siteida, siteheightAm + 0.5);
                            ...
                            if (Math.Round(deviceheightBm) < Math.Round(xObstm))
                            {
                                SetSiteHeight(puid, siteidb, xObstm - locheightBm);
                                linksmeet = false;
                                internalSitesModified.Add(siteidb);
                            }
                            else
                            {
                                SetSiteHeight(puid, siteidb, siteheightBm + 0.5);
                                linksmeet = false;
                                internalSitesModified.Add(siteidb);
                            }
                        }
            ...
            return String.Join(",",
                internalSitesModified.Distinct().OrderBy(x => x).ToArray()) +
                "@" + String.Join(",",
            ...
            CreateLinkOptions options)
        {
            ...
            string sql = "INSERT INTO puserslinks (" +
                ...
                "siteida, locheighta, bearinga, channelwidtha, frequencya, " +
            ...
            sql += "ST_AsText(sitea.wkb_geometry) AS locaheighta, ";
            sql += "CASE WHEN " +
