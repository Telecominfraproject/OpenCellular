...
static struct irq_domain *gpio_domain;
static __read_mostly u8 octeon_irq_ciu_to_irq[8][64];

unsigned int octeon_gpio_irq(int gpio);
				...
				unsigned int *out_type)
{
	...
	*out_hwirq = gpiod->base_hwirq + pin;
    ...
    printk("Irq domain base irq number for GPIO %u\n", irq_find_mapping(gpio_domain, 17) );
	return 0;
}
...
unsigned int octeon_gpio_irq(int gpio)
{
    struct octeon_irq_gpio_domain_data *gpiod;
    unsigned int irq = 0;
    gpiod = gpio_domain->host_data; 
    irq = irq_find_mapping(gpio_domain, (gpiod->base_hwirq + gpio));
    printk("octeon_gpio_irq gpio number %d irq number for GPIO %u\n", gpio, irq);
    return irq;
}

EXPORT_SYMBOL(octeon_gpio_irq);
...
static void __init octeon_irq_init_ciu(void)
{
	...
	if (gpio_node) {
		...
		if (gpiod) {
			...
			gpiod->base_hwirq = 16;
			gpio_domain = irq_domain_add_linear(gpio_node, 16, &octeon_irq_domain_gpio_ops, gpiod);
