...
/*****************************************************************************
 **    FUNCTION NAME   : i2c_reg_write_16bit_address
 **
 **    DESCRIPTION     : Writing device register over i2c bus.
 **
 **    ARGUMENTS       : I2C handle, device address, register address and value.
 ...
 **
 *****************************************************************************/
ReturnStatus i2c_reg_write_16bit_address(I2C_Handle i2cHandle,
                                         uint8_t deviceAddress,
                                         uint16_t regAddress,
                                         uint16_t value, /* Changed to 16bit */
                                         uint8_t numofBytes)
{
    ReturnStatus status = RETURN_OK;
    uint8_t txBuffer[4];
    ...
    txBuffer[2] = (value & 0xFF00) >> 8;
    txBuffer[3] = (value & 0x00FF);
    i2cTransaction.slaveAddress = deviceAddress;
    i2cTransaction.writeBuf = txBuffer;
    i2cTransaction.writeCount = numofBytes + 2;
    i2cTransaction.readBuf = NULL;
    i2cTransaction.readCount = 0;
    if (I2C_transfer(i2cHandle, &i2cTransaction)) {
        status = RETURN_OK;
...
/*****************************************************************************
 **    FUNCTION NAME   : i2c_reg_read_16bit_address
 **
 **    DESCRIPTION     : Reading device register over i2c bus.
 **
 **    ARGUMENTS       : I2C handle, device address, register address and value.
 ...
 **
 *****************************************************************************/
ReturnStatus i2c_reg_read_16bit_address(I2C_Handle i2cHandle,
                                        uint8_t deviceAddress,
                                        uint16_t regAddress, uint32_t *value,
                                        uint8_t numofBytes)
{
    ReturnStatus status = RETURN_OK;
...
/*****************************************************************************
 **    FUNCTION NAME   : i2c_reg_read_32bit_address
 **
 **    DESCRIPTION     : Reading device register over i2c bus.
 **
 **    ARGUMENTS       : I2C handle, device address, register address and value.
 ...
 **
 *****************************************************************************/
ReturnStatus i2c_reg_read_32bit_address(I2C_Handle i2cHandle,
                                        uint8_t deviceAddress,
                                        uint32_t regAddress, uint32_t *value,
                                        uint8_t numofBytes, uint8_t write_count)
{
    ReturnStatus status = RETURN_OK;
    uint8_t txBuffer[4] = { 0 };
    uint8_t rxBuffer[5] = { 0 };
    txBuffer[0] = (regAddress & 0x000000FF);
    txBuffer[1] = (regAddress & 0x0000FF00) >> 8;
    txBuffer[2] = (regAddress & 0x00FF0000) >> 16;
    txBuffer[3] = (regAddress & 0xFF000000) >> 24;
    I2C_Transaction i2cTransaction;
    i2cTransaction.slaveAddress = deviceAddress;
    i2cTransaction.writeBuf = &txBuffer;
...
/*****************************************************************************
 **    FUNCTION NAME   : i2c_reg_write_32bit_address
 **
 **    DESCRIPTION     : Writing device register over i2c bus.
 **
 **    ARGUMENTS       : I2C handle, device address, register address and value.
 ...
 **
 *****************************************************************************/
ReturnStatus i2c_reg_write_32bit_address(I2C_Handle i2cHandle,
                                         uint8_t deviceAddress,
                                         uint32_t regAddress, uint32_t value,
                                         uint8_t numofBytes,
                                         uint8_t write_count)
{
    ...
    if (write_count == 4) {
        ...
        txBuffer[2] = (regAddress & 0x00FF0000) >> 16;
        txBuffer[3] = (regAddress & 0xFF000000) >> 24;
        txBuffer[4] = (value & 0x000000FF);
        txBuffer[5] = (value & 0x0000FF00) >> 8;
        txBuffer[6] = (value & 0x00FF0000) >> 16;
        txBuffer[7] = (value & 0xFF000000) >> 24;
    }
    if (write_count == 2) {
        txBuffer[0] = (regAddress & 0x000000FF);
        txBuffer[1] = (regAddress & 0x0000FF00) >> 8;
        txBuffer[2] = (value & 0x000000FF);
        txBuffer[3] = (value & 0x0000FF00) >> 8;
    }
    if (write_count == 1) {
        txBuffer[0] = (regAddress & 0x000000FF);
        txBuffer[1] = (value & 0x000000FF);
        txBuffer[2] = (value & 0x0000FF00) >> 8;
    }
    i2cTransaction.slaveAddress = deviceAddress;
    i2cTransaction.writeBuf = txBuffer;
    i2cTransaction.writeCount = write_count + numofBytes;
    i2cTransaction.readBuf = NULL;
    i2cTransaction.readCount = 0;
    if (I2C_transfer(i2cHandle, &i2cTransaction)) {
        status = RETURN_OK;
