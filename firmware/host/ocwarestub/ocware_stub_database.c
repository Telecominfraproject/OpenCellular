...
ocware_stub_ret ocware_stub_set_i2cinfo(int16_t gStructIndex, char *payload)
{
    ...
    if (I2CInfo[index]->numOfBytes == 1) {
        ...
        I2CInfo[index]->regValue =
            ((uint8_t)payload[pos + 3] << 0x08) + payload[pos + 4];
        payload[pos + 4] = (uint8_t)(I2CInfo[index]->regValue & MASK_LSB);
        payload[pos + 3] =
    ...
    for (component = subSystem->components; component && component->name;
        ...
        while (subComponent && subComponent->name) {
            ...
            if (driver != NULL) {
                ...
                if (strncmp(component->name, "I2C", strlen("I2C")) == 0) {
        ...
        for (component = subSystem->components; component && component->name;
            ...
            if (devDriver != NULL) {
                ...
                while (param && param->name) { /*Parameter loop */
                    ...
                    alertGlobalData[gStructIndex].paramId = pow(2, count);
                    ...
                    alertGlobalData[gStructIndex].msgtype =
                        OCMP_MSG_TYPE_CONFIG;
                    alertGlobalData[gStructIndex].data = malloc(
                        sizeof(char) * alertGlobalData[gStructIndex].paramSize);
            ...
            while (subComponent && subComponent->name) {
                ...
                while (param && param->name) { /*Parameter loop */
                    alertGlobalData[gStructIndex].subsystemId = subsystemNbr;
                    alertGlobalData[gStructIndex].componentId = componentNbr;
                    alertGlobalData[gStructIndex].paramId = pow(2, alertCount);
                    alertGlobalData[gStructIndex].paramSize =
                        ocware_stub_get_paramSize(DATA_TYPE_MAP[param->type],
                                                  OCMP_MSG_TYPE_CONFIG,
                                                  param->name);
